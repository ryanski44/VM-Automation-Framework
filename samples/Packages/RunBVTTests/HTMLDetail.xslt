...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/TR/html4/strict.dtd">	
	...
	<xsl:template match="/">
		...
		<div id="report">
				...
				<table>
					<tbody>
						<tr>
							...
							<td><xsl:value-of select="count($tests.root//test-suite[@type = 'Assembly'])"/></td>
						</tr>
						<tr>
			...
			<div id="details">
				...
				<xsl:apply-templates select="$tests.root//test-suite[@type = 'Assembly']" />
			</div>
		</div>
	</xsl:template>
				...
				<table><tr><td>
					<xsl:choose>
						...
						<xsl:when test="$ignored.count > 0">
							...
							<div style="background-color: yellow; width: 16px; height: 16px;">
	...
	<xsl:template match="test-suite">
		...
		<xsl:variable name="testName" select="concat(@name,generate-id())"/>
		<tr>
